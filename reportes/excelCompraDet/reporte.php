...
if(isset($_POST['generar_reporte']))
{
	...
	header('Content-Disposition: attachment; filename="Reporte_Fechas_Compra_Detalle.xls"');

	// SALIDA DEL ARCHIVO
	$salida=fopen('php://output', 'w');
	// ENCABEZADOS
	fputcsv($salida, array('Numero de O.C.', 'Codigo Producto', 'Nombre Producto', 'Codigo Cliente', 'Nombre Cliente','Ruc','Precio', 'Iva 10%', 'Iva 05%', 'Fecha Facturacion'));
	// QUERY PARA CREAR EL REPORTE
	$reporteCsv=$conexion->query("SELECT *  FROM detalle_compra,productos where (fecha BETWEEN '$fecha1' AND '$fecha2') and detalle_compra.id_producto=productos.codigo_producto ORDER BY numero_factura");
