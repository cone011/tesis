...
{
$accion='MODIFICADO';
$insert_tmp=mysqli_query($con, "INSERT INTO detalle_compra (numero_factura, id_producto,cantidad,precio_venta,accion) VALUES ('$numero_factura','$id','$cantidad','$precio_venta','$accion')");
$insert_auditoria=mysqli_query($con, "INSERT INTO audidetalle_compra (numero_factura, id_producto,cantidad,precio_venta,accion) VALUES ('$numero_factura','$id','$cantidad','$precio_venta','$accion')");
...
<table class="table">
<tr>
	<th class='text-center'>NRO O.C.</th>
	<th>DESCRIPCION</th>
...
<?php
    ...
    $monto5=0;
    $monto0=0;
	$sql=mysqli_query($con, "select * from nproveedor, detalle_np where nproveedor.numero_factura=detalle_np.numero_factura and nproveedor.id_factura='$id_factura'");
	while ($row=mysqli_fetch_array($sql))
	{
	/*$id_detalle=$row["id_detalle"];
	...
	$estado_factura=$row['estado_factura'];
	$sql_producto=mysqli_query($con, "select * from proveedores, nproveedor where nproveedor.id_cliente=proveedores.id_cliente");
    while ($rw=mysqli_fetch_array($sql_producto))
    {
        $nombre=$rw['nombre_cliente'];
         $ruc_cliente=$rw['telefono_cliente'];
        $dato=$nombre.'  '.$ruc_cliente;
        $dato_ruc=$rw['ruc_cliente'];
    }
