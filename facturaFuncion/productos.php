		    ...
		    <div class="btn-group pull-right">
		    	 <?php   
						...
						while ($rw=mysqli_fetch_array($sql_factura)){
								$id_factura=$rw["factura_final"];
					    }

					    $sql_factura_venta=mysqli_query($con,"select * from venta order by numero_factura desc limit 1");
						while ($raw=mysqli_fetch_array($sql_factura_venta)){
								$factura=$raw["numero_factura"];
					    }
					?>
				<?php if($id_factura==$factura){ ?>	

				<?php }else{ ?>	
				    <button type='button' class="btn btn-info" data-toggle="modal" data-target="#nuevoProducto"><span class="glyphicon glyphicon-plus" ></span> Nuevo Cierre</button>
				<?php  } ?>
			</div>
			<h4><i class='glyphicon glyphicon-search'></i> Buscar Cierre</h4>
		</div>
		<div class="panel-body">
			...
			<form class="form-horizontal" role="form" id="datos_cotizacion">
						...
						<div class="form-group row">
							<label for="q" class="col-md-2 control-label">Fecha/Factura/#Cierre</label>
							<div class="col-md-5">
								<input type="text" class="form-control" id="q" placeholder="Fecha/Factura/#Cierre" onkeyup='load(1);'>
							</div>
							<div class="col-md-3">
